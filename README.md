...
![encroachment classification figure](encroachment_classification.png)
...
| `x_3d`, `y_3d`             | float (meters)  | Georeferenced planar (ground-plane) position of the object, in a local metric coordinate system — e.g. obtained via camera-calibration/homography from image pixel coordinates. The code assigns the CRS label `EPSG:3857` to the resulting GeoDataFrame purely so `movingpandas` treat distances as metric — it does not reproject or otherwise require actual Web-Mercator coordinates. |
